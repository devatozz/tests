...
export default function Pools() {
    ...
    const handleToken1AmountChange = async (amount) => {
        ...
        const token2Amount = calculateTokenAmount(
            poolInfo.token1 === token1Name
                ? poolInfo.reserve1
                : poolInfo.reserve2,
            poolInfo.token2 === token2Name
                ? poolInfo.reserve2
                : poolInfo.reserve1,
            amount,
            token1Name
    ...
    useEffect(() => {
        ...
        if (
            (token1Name &&
                token1Balance.lt(
                    ethers.utils.parseUnits(token1Amount, tokens.obj[token1Name]?.decimals))
                ) ||
            (token2Name &&
                token2Balance.lt(
                    ethers.utils.parseUnits(token2Amount, tokens.obj[token2Name]?.decimals))
                )
        ) {
    ...
    const calculateTokenAmount = (
        ...
        const amount1BN = ethers.utils.parseUnits(token1Amount, tokens.obj[tokenName].decimals);
    ...
    return (
        <Center>
            <Center borderRadius={"md"} bgColor="#171E2E" px={4} py={6} color='white'>
