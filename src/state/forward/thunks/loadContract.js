...
const loadContracts = createAsyncThunk("forward/contract", async (_payload, { getState }) => {
    ...
    if (state.chain.account) {
        const { dexAddress, factoryAddress } = forwardConfig[state.chain.selectedChain];
        ...
        const provider = new ethers.providers.JsonRpcProvider(forwardConfig[selectedChain].rpcAddress);
