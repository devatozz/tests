...
const AirdropPage = () => {
  ...
  return (
    <Box width={"full"} bgGradient="linear(180deg, #3146C6 0%, #18215D 100%)">
      <Container
        ...
        <VStack spacing="24px" mt={14}>
          ...
          <Flex width={"full"} flexWrap="nowrap">
            <Text
              ...
              color="white"
            >
              Referal link: {`https://pira.finance/Signup?ref=${account}`}
            </Text>
            <CopyIcon
              onClick={copyRefLink}
              fontSize="2xl"
...
const AirdropPage = () => {
  ...
  const airdopState = useSelector((state) => state.airdrop);
  const taskList = useMemo(() => airdopState.taskList, [airdopState.taskList]);
  //TODO: MAP TASKLIST
  useEffect(() => {
  ...
  return (
    ...
    >
      {address === "" ? (
        ...
        <>
          {airdopState.isLoading && <CicularLoading />}
          ...
          <Text fontSize="3xl" textAlign="center" color="white" my={6}>
            ...
            transaction fees will be used for buyback when the token is listed
            on CEX
          </Text>
